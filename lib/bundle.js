...
const Game = function(opts) {
  ...
  let n = opts.level + 5;
  while (this.graph.intersectionPositions.length < 3) {
    this.graph.reset();
...
Graph.prototype.setIntersectionPoints = function() {
  ...
  Util.uniqPositions(this.intersectionPositions).forEach(pos => {
    let newPoint = new Point({pos: pos});
...
Line.prototype.hasPos = function (pos) {
  ...
  const isXInRange = Util.inRange(this.startX, this.endX, x) &&
    Util.inRange(0, 600, x);
  const isYInRange = Util.inRange(this.startY, this.endY, y) &&
    Util.inRange(0, 600, x);

  //check that y === mx + b
  const isOnLine = Boolean(
    Util.inRange(-1, 1, (y - Math.floor((this.slope * x) + this.yIntercept)))
  );
